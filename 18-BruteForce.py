...
def Brute_Zip():
    # variable for user input path of the password list and the file to brute force
    Password_List = input("Input file path of password list: ")
    Zipped_File = input("Input file path of zipped file: ")
    
    #with the password list open, read it as file
    with open(Password_List, "r") as file:
        # for every line (referred to as password variable) in the file
        for password in file:
            # strips and trailing characters like extra spaces and saves it as variable password
            password = password.strip()
            try:
                # with the zipfile module using the variable for the path of the zip file, refer to the varaialbe as zip
                with zipfile.ZipFile(Zipped_File) as zip:\
                    # extract the password in bytes with utf 8 encoding
